...
import { InfoContainer } from "../../realEstate/realEstateCard/RealEstateCard.styled";
import { CardContainer, CardsWrapper, ChartsContainer, ChartWithInputs } from "../../realEstate/EnergyGraph/EnergyGraph.styled";
...
export default function EnergyCityGraph() {
    ...
    function getTransactionsToGrid() {
        EnergyTransactionService.getEnergyTransactionsToGridByCity(String(decodedCity), period).then(response => {
    ...
    function getTransactionsFromGrid() {
        console.log(decodedCity);
        EnergyTransactionService.getEnergyTransactionsFromGridByCity(String(decodedCity), period).then(response => {
            let transactions: EnergyTransaction[] = response.data;
    ...
    const onMessageReceive = (message: Frame) => {
        ...
        // Check conditions
        if (newMeasure.realEstateCity === decodedCity && selectedPeriod.label.match("1 Hour")) {
        ...
        // Check conditions
        if (newMeasure.realEstateCity === decodedCity && selectedPeriod1.label.match("1 Hour")) {
